...
function prop_adjoint(node::Op.Type)
    @match node begin
        ...
        Op.Adjoint(Op.Subscript(base, idx)) => Op.Subscript(base', idx)
        Op.Adjoint(Op.Sum(region, term)) => Op.Sum(region, term')
        Op.Adjoint(Op.Prod(region, term)) => Op.Prod(region, term')
