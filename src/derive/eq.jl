...
function eq_derive_variant_field(variant_type)
    ...
    for (idx, type) in enumerate(fieldtypes)
        ...
        @gensym lhs rhs
        lhs_val = xcall(Reflection, :variant_getfield, :lhs, Val(variant_type.tag), idx)
        rhs_val = xcall(Reflection, :variant_getfield, :rhs, Val(variant_type.tag), idx)
    ...
    return if hash_cache_idx > 0
        ...
        lhs_val = xcall(
            Reflection, :variant_getfield, :lhs, Val(variant_type.tag), hash_cache_idx
        )
        rhs_val = xcall(
            Reflection, :variant_getfield, :rhs, Val(variant_type.tag), hash_cache_idx
        )
        quote
            $lhs_hash = $lhs_val
            $rhs_hash = $rhs_val
